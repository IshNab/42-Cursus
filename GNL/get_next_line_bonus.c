...
/*   Updated: 2025/02/07 16:32:14 by inabakka         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*read_from_file(char *basin_buffer, int fd)
{
	...
	return (free(temp_buffer), basin_buffer);
}
...
char	*get_next_line_bonus(int fd)
{
	...
	if (!basin_buffer[fd] || !*basin_buffer[fd])
		return (free(basin_buffer[fd]), NULL);
	newline_pos = ft_strchr(basin_buffer[fd], '\n');
	if (!(newline_pos))
	{
		line = ft_strdup(basin_buffer[fd]);
		return (free(basin_buffer[fd]), basin_buffer[fd] = NULL, line);
	}
